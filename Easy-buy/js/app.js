// create a function for loading api data.
const loadProducts = () => {
...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    // dynamically update rating value.
    rating = product.rating.rate;
    // single card image url.
    ...
    div.innerHTML = `<div class="single-product bg-white">
      <div>
        ...
        <h5><b>${product.title.slice(0, 20)}</b></h5>
        <p> <b>Category:</b> ${product.category}</p>
        <h6> <b>Price: $ ${product.price}</b> </h6>
        <p class="mb-1">Rating: <span class="fw-bold">${product.rating.rate}</span> <span id="stars" >${getStars(rating)}</span> <span class="text-secondary fw-bold"></span></p>
      <p class="mb-1">Total Rating: <span class="fw-bold">( <i class="fa fa-user" aria-hidden="true"></i> ${product.rating.count} )</span> </p>
      
      <div class="footer-card d-flex-column align-items-center">
        <button onclick="addToCart(${product.id},${product.price})" id="addToCart-btn" class="buy-now btn btn-secondary">Add to Cart <i class="fa fa-cart-plus text-dark" aria-hidden="true"></i></button>
...
// initialize products count variable and set it to 0
let count = 0;
// add to cart function 
const addToCart = (id, price) => {
  count = count + 1;
  // calling updatePrice function to update price in cart
  updatePrice("price", price);
  // calling updateTaxAndCharge function to update tax and charge in cart
  updateTaxAndCharge();
  // calling updateTotal function to update total in cart
  updateTotal();
  document.getElementById("total-Products").innerText = count;
};

// make getInputValue function to get the input value
...
// calling loadProducts function to load the products
loadProducts();

// star icon update 
document.getElementById("stars").innerHTML =getStars(rating);

// function for dynamic star icon
